# Note: expiration_time logic at the bottom of this code sample is hacky and needs to be cleaned up.
...
base64_key = "abc123"  # base64-encoded private key
signature_algorithm = "Ed25519" # Signature algo
url_prefix = "https://media.example.com/videos/" # URL path to protect
expiration_time = datetime.datetime.utcfromtimestamp(1708616344) # This is hacky and needs to be cleaned up. Instructions: Adjust epoch to line up with UTC time relative to your timezone. For example, UTC is 5 hours ahead of ETC. Setting epoch 5 hours ahead allows this to work.

# Prints signed token to console
print(sign_token(url_prefix=url_prefix, expiration_time=expiration_time, base64_key=base64_key, signature_algorithm=signature_algorithm))